...
def shuffle(x, g):
    batch_size, channels, heights, width, depth = x.size()
    per_group = channels // g
    x = x.view(batch_size, g, per_group, heights, width, depth)
    x = torch.transpose(x, 1, 2).contiguous()
    x = x.view(batch_size, -1, heights, width, depth)
    return x

class MFunit_sf(nn.Module):
    ...
    def forward(self, x):
        ...
        x3_ = shuffle(x3, 16)
        x4 = self.conv3x3x3_m2(x3_)
        #todo
        #x4 = self.conv3x3x3_m2(x4)
...
class MFSNet(nn.Module): #
    ...
    def __init__(self, c=4,n=32,channels=128,groups = 16,norm='bn', num_classes=4):
        ...
        self.encoder_block2 = nn.Sequential(
            MFunit_sf(n, channels, g=groups, stride=2, norm=norm),# H//4 down
            MFunit_sf(channels, channels, g=groups, stride=1, norm=norm),
            MFunit_sf(channels, channels, g=groups, stride=1, norm=norm)
        )
        #
        self.encoder_block3 = nn.Sequential(
            MFunit_sf(channels, channels*2, g=groups, stride=2, norm=norm), # H//8
            MFunit_sf(channels * 2, channels * 2, g=groups, stride=1, norm=norm),
            MFunit_sf(channels * 2, channels * 2, g=groups, stride=1, norm=norm)
        )

        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            MFunit_sf(channels*2, channels*3, g=groups, stride=2, norm=norm), # H//16
            MFunit_sf(channels*3, channels*3, g=groups, stride=1, norm=norm),
            MFunit_sf(channels*3, channels*2, g=groups, stride=1, norm=norm),
        )

        self.upsample1 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False) # H//8
        self.decoder_block1 = MFunit_sf(channels*2+channels*2, channels*2, g=groups, stride=1, norm=norm)

        self.upsample2 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False) # H//4
        self.decoder_block2 = MFunit_sf(channels*2 + channels, channels, g=groups, stride=1, norm=norm)
...
if __name__ == '__main__':
    ...
    model = MFSNet().cuda()
    input = torch.rand((1, 4, 128, 128, 128), device=cuda0)
